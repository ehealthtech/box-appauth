...
// @param opts {Object} The original options sent to factory.js
//
...
console.log(err, body)
        if(_.isObject(err)) {
            if(err.code === 'ETIMEDOUT') {
                message += util.format('Call timed out. Connection status: <%s>', err.connect);
            }
        }
        ...
        if(message) {
            logger.error('appauth call error -> ' + message)
        }

        return JSON.stringify({
            headers: headers,
            statusCode: statusCode,
            message: message || 'General Call Error'
        });
    };
	...
	env.prepare = function(callObj) {
        ...
        callObj.timeout = opts.apiCallTimeoutMs;
    ...
    // Box uses RFC 3339 timestamps. Expect a @moment time object, and
    // return the converted timestamp.
    //
    env.getTimestampForMoment = function(mom) {
