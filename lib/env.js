...
module.exports = function(Token) {
    ...
    var prepError = function(obj, strMsg) {
        ...
        return {
            error: obj instanceof Error ? obj : new Error(err),
            message: strMsg
        };
    };
	...
	env.complete = function(cb) {
		return function(err, res, body) {
            ...
            // no callback, and happens "in the background". See ./env.js
            //
            Token.refreshIfExpiring();
			...
			if(!body) {
                if(~[401,403].indexOf(statusCode)) {
                    return cb(prepError(headers['www-authenticate']));
                }
			} else {
