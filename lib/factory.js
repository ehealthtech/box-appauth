			...
			return new Promise(function(resolve, reject) {
				...
				request.post({
					...
					if(err) {
						...
						debug && console.warn('Unable to revoke token ->', accessToken);
					...
					debug && console.info('Token ->', accessToken, 'has been revoked');
		...
		this.refreshIfExpiring = function() {
			...
			if(!isRefreshing && doRefresh) {
                ...
                .then(function(tok) {
                    ...
                    debug && console.info("---> Token has been refreshed. New:", tok);
                })
                .catch(function(err) {

                    debug && console.warn("---> Token could not be refreshed", err);
                })
                .finally(function() {
                    isRefreshing = false;
                });
