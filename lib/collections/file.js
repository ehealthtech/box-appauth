...
module.exports = function(env) {
	...
	return {
		...
		download: function(args, cb) {
			...
			var url = env.prepare({
				url: util.format('https://api.box.com/2.0/files/%d/content', env.toNumberOrThrow(args.id, 'file:download#fileId'))
			});

			// If the response expects a stream back just call the callback with our stream
			// Otherwise call our internal callback handler.
			if(args.asStream){

				var myStream = request(url);

				// Due to this issue pause immediatly https://github.com/request/request/issues/887
				myStream.on('response', function pauseBoxStream(streamResponse){
					//streamResponse.pause();
				});

				cb(null, myStream);
			} else {
				// call our provided custom callback to handle the response
				request.get(url, env.complete(cb, true));
			}
