...
module.exports = function(args) {
	...
	if(!_.isPlainObject(args)) {
		throw new Error('ehealth-box-appauth factory received non-Object as argument');
	}
	...
	if(!~supportedAlgorithms.indexOf(algorithm)) {
		console.warn('ehealth-box-appauth #algorithm must be one of ' + supportedAlgorithms.toString() + '. Received: ' + algorithm + '. Will use ' + defaultAlgorithm);
		
		algorithm = defaultAlgorithm;
	}

	if(!issuer) {
		throw new Error('ehealth-box-appauth did not receive #issuer');
	}
	
	if(!subject) {
		throw new Error('ehealth-box-appauth did not receive #subject');
	}
	
	if(!~validSubjectTypes.indexOf(subjectType)) {
		throw new Error('ehealth-box-appauth #subjectType must be one of ' + validSubjectTypes.toString() + '. Received: ' + subjectType);
	}
	
	if(!publicKey) {
		throw new Error('ehealth-box-appauth did not receive a #publicKey');
	}
	
	if(!privateKey) {
		throw new Error('ehealth-box-appauth did not receive a #privateKey');
	}

    if(!publicKeyId) {
        throw new Error('ehealth-box-appauth did not receive a #publicKeyId');
    }
	
	if(!clientId) {
		throw new Error('ehealth-box-appauth did not receive a #clientId');
	}
	
	if(!clientSecret) {
		throw new Error('ehealth-box-appauth did not receive a #clientSecret');
	}
