...
module.exports = function(args) {
    ...
    var callRetryMax = args.callRetryMax || 10;

    // Must be >= 5 and <= 50 minutes
    //
    var minutesUntilTokenRefresh = +args.minutesUntilTokenRefresh;
    minutesUntilTokenRefresh = minutesUntilTokenRefresh < 5
                                ? 5
                                : minutesUntilTokenRefresh > 50
                                ? 50
	...
	return new Promise(function(resolve, reject) {
		build({ 
			...
			expiresIn: 59,
            iat: Date.now() - 10000, // Attempt to ensure that the nbf/iat is
            nbf: Date.now() - 10000, // accepted by Box. Optional. Hopefully can be removed.
			headers: {
				typ: 'JWT',
				alg: algorithm,
                kid: publicKeyId
		...
		privateKey, {
            ...
            callRetryMax : callRetryMax || 10,
            minutesUntilTokenRefresh : minutesUntilTokenRefresh || 30
        })
		.then(resolve)
		.catch(reject);
	});
